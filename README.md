在对金融数据进行处理时，常遇到金融数据软件导出功能不够智能的情况，因此在Automation-toolbox中收录自己写的一些改进效率的程序。目前，本项目当前包含“Download detailed statistical data from Choice Terminal”、“Extract certain keywords from PDF documents online”和“Extract data from Eastmoney's webpage”三个子项目。
...
“Extract data from Eastmoney's webpage”的脚本（Crawling through multiple pages on Eastmoney Pro.py）主要运用selenium识别东方财富的动态网页内容，在模拟的浏览器中使用JavaScript的功能实现定位、提取网页元素、滚动、翻页等操作。最后通过Pandas将提取出来的数据（新股股票代码、新股股票简称）保存在表格中。
当程序打开网页时，请记得在程序设置的加载时间内手动关闭全屏弹窗广告，以免干扰程序运行。相关等待时间参数可按需修改以加快效率。
...
The “Extracting Data from Eastmoney Webpages” script (Crawling through multiple pages on Eastmoney Pro.py) primarily uses Selenium to identify dynamic content on Eastmoney webpages. Within a simulated browser environment, it employs JavaScript functionality to perform operations such as locating and extracting webpage elements, scrolling, and pagination. Finally, Pandas saves the extracted data (new stock ticker codes and abbreviations) into a spreadsheet.
When running the program to open the webpage, manually close any full-screen pop-up ads within the program's designated loading time to prevent interference with its operation. The relevant wait time parameters can be adjusted as needed to enhance efficiency.
